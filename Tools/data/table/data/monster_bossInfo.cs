...
namespace DesignTable
{
    ...
    public class monster_bossInfos
    {
        ...
        { 
            foreach(monster_bossInfo info in dataInfo)
            {
            ...
            dataInfo.Add(new monster_bossInfo(mon_id,mon_pathId,mon_type,mon_spawnId,mon_spawnTime,mon_spawnDayNight));
            return true;
        }
        ...
        public void Initialize() 
        {
            foreach(var data in dataInfo)
            {
                ...
                if (datas.ContainsKey(bytes))
                    continue;
                datas.Add(bytes,new monster_bossInfo(data.mon_id,data.mon_pathId,data.mon_type,data.mon_spawnId,data.mon_spawnTime,data.mon_spawnDayNight));
        ...
        public ArraySegment<byte> GetIdRule(short mon_id)
        {
            ushort total = 0;
            ushort count = 0;
                      total += sizeof(short);
