...
namespace DesignTable
{
    ...
    public class passiveInfos
    {
        ...
        public void Initialize() 
        {
            foreach(var data in dataInfo)
            {
                ...
                if (datas.ContainsKey(bytes))
                    continue;
                datas.Add(bytes,new passiveInfo(data.passive_Id,data.status_Type,data.status_Arg,data.passive_name,data.image_Res));
        ...
        public ArraySegment<byte> GetIdRule(short passive_Id)
        {
            ushort total = 0;
            ushort count = 0;
                      total += sizeof(short);
